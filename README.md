...
このリポジトリは授業用の Stretch3 派生版です。ML2Scratch の画像分類で最高確信度を正しく比較できるよう、ビルド時に不足している `topConfidence` の更新処理を追加します。また、ステージ画像の学習時は MobileNet の画像特徴に学習対象スプライトの `x`・`y` 座標を加え、迷路内の場所を区別しやすくしています。Scratch 本体の renderer canvas を明示して、別の canvas を誤って使う可能性も防ぎます。

カメラ画像の学習方法は変更しません。授業用サイトへ元サイトのアクセス解析IDを持ち込まないよう、Google Analytics の追加処理も無効化しています。

公開用のビルド成果物は `main` ブランチへ配置されます。
